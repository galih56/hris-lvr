...
function Calendar({
  ...
  return (
    <DayPicker
      ...
      components={{
        Caption: (props) => (
          <CustomCaption month={props.displayMonth} />
        ),
        Chevron: (props) => {
...
export function CustomCaption({ month }: CustomCaptionProps) {
  const { goToMonth, nextMonth, previousMonth } = useNavigation();

  const handleMonthChange = (event: React.ChangeEvent<HTMLSelectElement>) => {
    const newMonth = new Date(month);
    newMonth.setMonth(parseInt(event.target.value, 10));
    goToMonth(newMonth);
  };
  ...
  return (
    <div className="flex items-center justify-center gap-4">
      ...
      <input
        ...
        className="w-18 border rounded px-2 py-1 text-sm"
      />
