...
type UpdateLeaveRequestProps = {
  leaveRequestId: string | undefined;
};

export const UpdateLeaveRequest = ({ leaveRequestId }: UpdateLeaveRequestProps) => {
  ...
  const leaveRequestQuery = useLeaveRequest({ leaveRequestId });
  const updateLeaveRequestMutation = useUpdateLeaveRequest({
    mutationConfig: {
      onSuccess: () => {
        addNotification({
          type: 'success',
          title: 'LeaveRequest Updated',
        });
      },
    },
  });
  ...
  if(!leaveRequest || leaveRequestQuery.isPending){
    return null
  }

  const isFetching = useIsFetching();
  const form = useForm<z.infer<typeof updateLeaveRequestInputSchema>>({
    resolver: zodResolver(updateLeaveRequestInputSchema),
    defaultValues : {
  ...
  async function onSubmit(values: z.infer<typeof updateLeaveRequestInputSchema>) {
    ...
    updateLeaveRequestMutation.mutate({ data : values, leaveRequestId : leaveRequest?.id!})
  }
